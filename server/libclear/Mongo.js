...
export function createPatient(patientInfo, callback) {
  MongoClient.connect(url, function (err, client) {
    assert.equal(null, err);
    client.db('ihc').collection('patients').insertOne({ info: patientInfo }, function (err, r) {
      assert.equal(null, err);
...
export function updateStatus(patientInfo, newStatus, callback) {
  MongoClient.connect(url, function (err, client) {
    ...
    client.db('ihc').collection('patients').updateOne({ info: patientInfo }, { $set: { status: newStatus } }, function (err, r) {
      assert.equal(null, err);
...
export function updateSoap(patientInfo, newSoap, callback) {
  MongoClient.connect(url, function (err, client) {
    ...
    client.db('ihc').collection('patients').updateOne({
      info: patientInfo
    }, intermediaryUpdate, function (err, r) {
      assert.equal(null, err);
      assert.equal(r.modifiedCount, 1);
      client.close();
      callback(r.result.ok === 1);
...
function addUpdate(update, updatesTimestampRef, groupId, database) {
  ...
  updateRef.set(update).catch(error => console.log('Error while calling Firebase set()', error));
...
function getUserKey(hash, groupId, database) {
  const usersRef = database.ref(`/groups/${groupId}/users`);
  return usersRef.orderByChild('data').equalTo(hash).once('value').then(snapshot => snapshot.val()).then(keyObj => keyObj ? Object.keys(keyObj)[0] : null);
}
...
export function addUpdates(updates, groupId, database, timestamp) {
  ...
  return addNewUsers(updates, groupId, database).catch(() => {
